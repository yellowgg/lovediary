...
public class DiaryController {
    ...
    public String findPage(@PathVariable("pageNum") Integer pageNum, @PathVariable("categoryId") String categoryId,
        ...
        try {
            ...
            //能走到这里，不出错肯定是有日记的。根据分类查询分页日记
            PageBean<Diary> diaryList = diaryService.findAllByCatePage(pageNum, pageSize, category);
            //传回前台
            model.addAttribute("diariesOfTopCate", diaryList);
            //顺便也把分类id传过去，因为翻页要用到
            ...
            BaseLogger.ERROR_LOGGER.error("根据分类分页查询日记失败", e);
            //跳转到错误页面
            try {
                response.sendRedirect(request.getContextPath() + "/other/error");
            } catch (IOException ex) {
    ...
    public String info(@PathVariable("diaryId") String diaryId, Model model, HttpServletResponse response, HttpServletRequest request) {
        try {
            //封装Diary
            ...
            BaseLogger.ERROR_LOGGER.error("获取日记详情失败", e);
            try {
                response.sendRedirect(request.getContextPath() + "/other/error");
            } catch (IOException ex) {
            }
    ...
    public void addDiary(Diary diary, HttpServletResponse response, HttpServletRequest request) {
        try {
            //createtime
            ...
            BaseLogger.ERROR_LOGGER.error("添加日记失败", e);
            try {
                response.sendRedirect(request.getContextPath() + "/other/error");
            } catch (IOException ex) {
            }
    ...
    public String modifyUI(@PathVariable("diaryid") String diaryId, Model model, HttpServletResponse response, HttpServletRequest request) {
        try {
            //封装Diary
            Diary diary = new Diary(diaryId);
            ...
            BaseLogger.ERROR_LOGGER.error("跳转到编辑日记失败", e);
            try {
                response.sendRedirect(request.getContextPath() + "/other/error");
            } catch (IOException ex) {
            }
    ...
    public void editDiary(Diary diary, HttpServletResponse response, HttpServletRequest request) {
        try {
    ...
    public String search(String searchKey, Model model, HttpServletResponse response, HttpServletRequest request) {
        try {
            //如果没有的话，就转发list
            ...
            BaseLogger.ERROR_LOGGER.error("搜索日记失败", e);
            try {
                response.sendRedirect(request.getContextPath() + "/other/error");
            } catch (IOException ex) {
            }
    ...
    public String findKeyPage(@PathVariable("pageNum") Integer pageNum, @PathVariable("searchKey") String searchKey,
                              Model model, HttpServletResponse response, HttpServletRequest request) {
        try {
            //设置每页的对象数量
            int pageSize = 10;
            ...
            BaseLogger.ERROR_LOGGER.error("根据搜索词分页查询日记失败", e);
            //跳转到错误页面
            try {
                response.sendRedirect(request.getContextPath() + "/other/error");
            } catch (IOException ex) {
